...
nCellsLength = int(
    input("Enter the number of Cells along the length for Meshing the Plate: ")
)
nCellsWidth = int(
    input("Enter the number of Cells along the width for Meshing the Plate: ")
)
...
dRightPattern = np.hstack(
    [xCentroidPattern[1:] - xCentroidPattern[0:-1], dRightBoundary]
)
dBottomPattern = np.hstack(
    [yCentroidPattern[1:] - yCentroidPattern[0:-1], dBottomBoundary]
)
...
topBoundaryID = np.hstack(
    [np.ones(nCellsLength), np.tile(np.zeros(nCellsLength), nCellsWidth - 1)]
)
bottomBoundaryID = np.hstack(
    [np.tile(np.zeros(nCellsLength), nCellsWidth - 1), np.ones(nCellsLength)]
)
...
for i in range(numCells):
    print(
        "%4i %5.0f %5.0f %5.0f %5.0f %5.0f %7.0f %5.0f"
...
Tleftrightshift = np.hstack(
    [tempLeft * np.ones([nCellsWidth, 1]), Tgrid, tempRight * np.ones([nCellsWidth, 1])]
)
Ttopbottomshift = np.vstack(
    [tempTop * np.ones([nCellsLength]), Tgrid, tempBottom * np.ones([nCellsLength])]
)
...
DA_left_boundary = (2 * conductivity * areaX / dLeftBoundary) * np.ones(
    [nCellsWidth, 1]
)
DA_right_boundary = (2 * conductivity * areaX / dRightBoundary) * np.ones(
    [nCellsWidth, 1]
)
DA_top_boundary = (2 * conductivity * areaY / dTopBoundary) * np.ones([nCellsLength])
DA_bottom_boundary = (2 * conductivity * areaY / dBottomBoundary) * np.ones(
    [nCellsLength]
)
...
heatFluxBottom = -np.multiply(
    np.multiply(DA_bottom_shift, deltaTbottom), normalsBottomGrid
)
...
for i in range(numCells):
    print(
        "%4i %7.1f %7.1f %7.1f %7.1f %7.1f %7.1f"
        % (
            i + 1,
            heatFluxLeftVector[i],
            heatFluxRightVector[i],
            heatFluxTopVector[i],
            heatFluxBottomVector[i],
            sourceVolVector[i],
            errorVector[i],
        )
    )
...
heatFluxBottomBoundaryTotal = np.sum(
    np.multiply(bottomBoundaryID, heatFluxBottomVector)
)
...
temperatureTopVector = np.hstack(
    [
        temperatureTopLeftCorner,
        tempTop * np.ones(nCellsLength - 1),
        temperatureTopRightCorner,
    ]
)

temperatureBottomVector = np.hstack(
    [
        temperatureBottomLeftCorner,
        tempBottom * np.ones(nCellsLength - 1),
...
Tnodes = np.vstack(
    [
        temperatureTopVector,
        np.hstack([temperatureLeftVector, Tinternalnodes, temperatureRightVector]),
...
ax.tick_params(
    which="both", direction="in", length=6, width=1.5, gridOn=False, pad=tickPad
)
