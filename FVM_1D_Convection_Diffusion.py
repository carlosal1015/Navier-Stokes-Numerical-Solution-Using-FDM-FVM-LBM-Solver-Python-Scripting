...
print(
    "Solving 1D Heat Convection Diffusion Equation with Dirichlet Boundary Condition\n"
)
...
heatfluxLeftCond = -1 * np.prod(
    [normalLeft, temperatureDifferenceLeft, DA_LeftFaces], 0
)
heatfluxRightCond = -1 * np.prod(
...
for i in range(nCell):
    print(
        "%4i %10.1f %12.1f %12.1f %11.1f %10.1f %7.1f"
        % (
            i + 1,
            heatfluxLeftCond[i],
            heatfluxRightCond[i],
...
if Pe[i] > 2:
    ...
    print(
        "Try to reduce Flow Velocity or Refine the Mesh or Switch to Upwind Scheme for Convection Term.\n"
    )
