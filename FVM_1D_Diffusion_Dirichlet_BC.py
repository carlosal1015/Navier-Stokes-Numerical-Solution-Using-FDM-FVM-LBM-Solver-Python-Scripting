...
nCells = int(
    input("Enter the number of Cells for Meshing the Rod: ")
)  # Number of Cells
...
for i in range(nCells):
    print(
        "Cell%2i %5.1f %5.1f %5.1f %7.1f %5.1f"
        % (i + 1, aL[i], aR[i], Sp[i], Su[i], aP[i])
    )
...
tempAnalytical = (
    tempLeft
    + ((tempRight - tempLeft) * (xAnalytical / barLength))
    + (heatSourcePerVol / (2 * cond))
    * xAnalytical
    * (barLength * numpy.ones(len(xAnalytical)) - xAnalytical)
)
...
ax.plot(
    xAnalytical, tempAnalytical, "k--", linewidth=2, label="Analytical", color=darkBlue
)
