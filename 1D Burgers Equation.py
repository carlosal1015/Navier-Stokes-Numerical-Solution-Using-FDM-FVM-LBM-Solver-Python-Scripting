...
from sympy.utilities.lambdify import (
    lambdify,
)  # translate sympy symbolic initial condition equation into a usable Python expression.
...
ufunc = lambdify(
    (t, x, nu), u
)  # putting value of t, x and nu in u and getting the output
...
)  # initial condition plot using our lambdify-ed function
...
u_analytical = numpy.asarray(
    [ufunc(nt * dt, xi, nu) for xi in x]
