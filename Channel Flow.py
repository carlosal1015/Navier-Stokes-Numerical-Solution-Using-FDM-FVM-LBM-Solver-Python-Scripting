...
def build_up_b(rho, dt, dx, dy, u, v):
    ...
    b[-1, 1:-1] = rho * (
        1
        / dt
        * ((u[0, 1:-1] - u[-2, 1:-1]) / (2 * dx) + (v[-1, 2:] - v[-1, :-2]) / (2 * dy))
        - ((u[0, 1:-1] - u[-2, 1:-1]) / (2 * dx)) ** 2
        - 2
        * ((u[-1, 2:] - u[-1, :-2]) / (2 * dy) * (v[0, 1:-1] - v[-2, 1:-1]) / (2 * dx))
        - ((v[-1, 2:] - v[-1, :-2]) / (2 * dy)) ** 2
    )
    ...
    b[0, 1:-1] = rho * (
        1
        / dt
        * ((u[1, 1:-1] - u[-1, 1:-1]) / (2 * dx) + (v[0, 2:] - v[0, :-2]) / (2 * dy))
        - ((u[1, 1:-1] - u[-1, 1:-1]) / (2 * dx)) ** 2
        - 2
        * ((u[0, 2:] - u[0, :-2]) / (2 * dy) * (v[1, 1:-1] - v[-1, 1:-1]) / (2 * dx))
        - ((v[0, 2:] - v[0, :-2]) / (2 * dy)) ** 2
    )
...
while residual > 0.01:  # Convergence Criteria
    ...
    u[-1, 1:-1] = (
        ...
        - vn[-1, 1:-1] * ((dt / dy) * (un[-1, 1:-1] - un[-1, :-2]))
        - ((dt / rho * 2 * dx) * (p[0, 1:-1] - p[-2, 1:-1]))
        + nu
        * (
            ((dt / dx**2) * (un[0, 1:-1] - 2 * un[-1, 1:-1] + un[-2, 1:-1]))
            + ((dt / dy**2) * (un[-1, 2:] - 2 * un[-1, 1:-1] + un[-1, :-2]))
        )
        + F * dt
    )
    ...
    u[0, 1:-1] = (
        ...
        - vn[0, 1:-1] * ((dt / dy) * (un[0, 1:-1] - un[0, :-2]))
        - ((dt / rho * 2 * dx) * (p[1, 1:-1] - p[-1, 1:-1]))
        + nu
        * (
            ((dt / dx**2) * (un[1, 1:-1] - 2 * un[0, 1:-1] + un[-1, 1:-1]))
            + ((dt / dy**2) * (un[0, 2:] - 2 * un[0, 1:-1] + un[0, :-2]))
        )
        + F * dt
    )
    ...
    v[-1, 1:-1] = (
        ...
        - vn[-1, 1:-1] * ((dt / dy) * (vn[-1, 1:-1] - vn[-1, :-2]))
        - ((dt / rho * 2 * dy) * (p[-1, 2:] - p[-1, :-2]))
        + nu
        * (
            ((dt / dx**2) * (vn[0, 1:-1] - 2 * vn[-1, 1:-1] + vn[-2, 1:-1]))
    ...
    v[0, 1:-1] = (
        ...
        - vn[0, 1:-1] * ((dt / dy) * (vn[0, 1:-1] - vn[0, :-2]))
        - ((dt / rho * 2 * dy) * (p[0, 2:] - p[0, :-2]))
        + nu
        * (
            ((dt / dx**2) * (vn[1, 1:-1] - 2 * vn[0, 1:-1] + vn[-1, 1:-1]))
