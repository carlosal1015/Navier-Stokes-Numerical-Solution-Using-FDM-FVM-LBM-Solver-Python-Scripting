...
heatfluxRight = -1 * np.prod(
    [normalRight, temperatureDifferenceRight, DA_RightFaces], 0
)
...
for i in range(nCell):
    print(
        "%4i %7.1f %7.1f %7.1f %7.1f"
        % (i + 1, heatfluxLeft[i], heatfluxRight[i], heatSource[i], heatBalanceError[i])
    )
print(lineSingle)
...
temperatureAnalytical = (
    (heatSourcePerVol / (2.0 * cond))
    * (barLength * barLength * np.ones(len(xAnalytical)) - np.square(xAnalytical))
    + (heatfluxLeftbc / cond) * (xAnalytical - barLength * np.ones(len(xAnalytical)))
    + tempRight
)
...
ax.plot(
    xPlotting, temperatureStack, "bo", linewidth=lineWidth, label="CFD", color=darkRed
)
ax.plot(
    xAnalytical,
    temperatureAnalytical,
